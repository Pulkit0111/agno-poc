...
def test_non_admin_can_still_list_schedules(client_and_db):
    """Reading the schedule list stays open to any authenticated user — only mutating/
    triggering schedules is admin-only."""
    from bott.skills import scheduling
    tc, db = client_and_db
    scheduling.create_security_digest(db, channel="#sec", cron="0 9 * * *")
    _as(tc)
    assert tc.get("/api/console/v1/schedules").status_code == 200
