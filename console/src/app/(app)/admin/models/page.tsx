...
export default function ModelsPage() {
  ...
  // Optimistic per-role overrides so a model switch shows up instantly instead
  ...
  // Codex is the only provider — every role picks from the one codex catalog.
  const catalog = data.catalogs.codex ?? [];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Pick which Codex model handles each job
        </p>
            ...
            <div key={role.key} className={`rounded-xl border bg-card p-4 shadow-sm ${isConflict ? "border-amber-500/40" : ""}`}>
              ...
              {catalog.length > 0 ? (
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {codex?.hint ?? "Connect ChatGPT above to pick a model."}
                </div>
              )}
            </div>
          );
