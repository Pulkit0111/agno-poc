...
| **4 — Connector framework** | partial | Registry is now **the** wiring path — `build_agent` sources every connector via `REGISTRY.all_tools()`; Jira/Confluence/Slack/Memra registered as thin org-credential entries; first **domain-delegated** connector shipped: **read-only Gmail** (per-call `delegated_user = verified caller`, `gmail.readonly`, read-only enforced structurally at toolkit construction; isolation gate proves "only my mail"). ✅ *Next in this phase:* Drive + Calendar (same delegated pattern), Gmail send behind the approval gate, Sentry connector. *Operator prerequisite:* a Workspace admin creates + authorizes the service account for `gmail.readonly` (like the GitHub App scope). |
| **5 — Skill engine + curator** | planned | Natural-language skill-authoring interview loop; the curator (usage tracking, retire stale, protect pinned) — the "do almost anything via skills" doubling-down. |
